...
function EliteBanner() {
  return (
    ...
    >
      <div>
        <div style={{ color: AMBER, fontWeight: 800, fontSize: 11, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>
          Elite Membership
        </div>
        <div style={{ color: "white", fontWeight: 700, fontSize: 18, lineHeight: 1.3, marginBottom: 8 }}>
          Unlock the full<br />club experience
        </div>
      ...
      <a
        href="#elite"
        style={{
          background: AMBER, color: DARK_TEXT, fontWeight: 700, fontSize: 13,
...
function LatestUpdates({ posts }: { posts: Post[] }) {
  const shown = posts.filter(p => !p.isEliteOnly).slice(0, 3);
