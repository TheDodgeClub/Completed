...
async function sendEliteWelcomeEmail(email: string, name: string): Promise<void> {
...
<div class="wrap">
  <div class="hdr">
    ...
    <p>You're now an Elite member of The Dodge Club</p>
  </div>
  <div class="body">
    <p>Hi ${name},</p>
    <p>Your Elite membership is now active — we're pumped to have you as one of our Elite members!</p>
    <div class="xp-box">
  ...
  const resp = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      to: [{ email, name }],
      subject: "⭐ Welcome to Elite — your 500 XP bonus is ready!",
      htmlContent: html,
    }),
  });
  if (!resp.ok) {
...
export async function activateElite(userId: number): Promise<void> {
  ...
  if (!user.eliteXpAwarded) {
    updates.bonusXp = (user.bonusXp ?? 0) + ELITE_XP_BONUS;
    updates.eliteXpAwarded = true;
  }

  await db.update(usersTable).set(updates).where(eq(usersTable.id, userId));
  logger.info({ userId, xpAwarded: !user.eliteXpAwarded }, "[activateElite] Elite activated");

  sendEliteWelcomeEmail(user.email, user.name).catch((err: unknown) =>
...
export async function deactivateElite(userId: number): Promise<void> {
  await db
    .update(usersTable)
    .set({ isElite: false, stripeSubscriptionId: null, pendingEliteCelebration: false })
